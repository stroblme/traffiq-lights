...
class traffiq_pqc():
    ...
    def classification_probability(self, data, variational):
        circuits = [self.qc.assign_parameters(
            self.circuit_parameters(d,variational)) for d in data]

        backend = q.BasicAer.get_backend('qasm_simulator')
        results =  q.execute(circuits, backend).result()

        classification = [self.label_probability(results.get_counts(c)) for c in circuits]
